-- Saga emotional logging.
-- Saga is global across LifeOS and stores only its own emotion entries.

CREATE TABLE IF NOT EXISTS saga_entries (
    id                 TEXT PRIMARY KEY,
    timestamp          TEXT NOT NULL,
    local_date         TEXT NOT NULL,
    emotion_family     TEXT NOT NULL CHECK (emotion_family IN (
...
CREATE INDEX IF NOT EXISTS idx_saga_entries_local_date
    ON saga_entries(local_date, timestamp);
CREATE INDEX IF NOT EXISTS idx_saga_entries_timestamp
    ON saga_entries(timestamp);
CREATE INDEX IF NOT EXISTS idx_saga_entries_family
    ON saga_entries(emotion_family, local_date);
