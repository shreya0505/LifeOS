-- Expand Saga Mood Meter coordinates from 10x10 to 14x14.
-- SQLite CHECK constraints cannot be altered in-place, so recreate the table.

PRAGMA foreign_keys = OFF;

DROP INDEX IF EXISTS idx_saga_entries_local_date;
DROP INDEX IF EXISTS idx_saga_entries_timestamp;
DROP INDEX IF EXISTS idx_saga_entries_quadrant;
...
CREATE TABLE saga_entries_new (
    ...
    energy             INTEGER NOT NULL CHECK (energy BETWEEN -7 AND 7 AND energy != 0),
    pleasantness       INTEGER NOT NULL CHECK (pleasantness BETWEEN -7 AND 7 AND pleasantness != 0),
