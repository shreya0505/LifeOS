"""Read-only analytical queries over pomodoro session data.

All functions accept sessions as a parameter — no internal I/O.
The caller loads data from whichever storage backend is in use.
"""
...
def get_quest_pomo_total(sessions: list[dict], quest_id: str) -> int:
    """Total completed work pomodoros across all sessions for a quest."""
    return sum(s.get("actual_pomos", 0) for s in sessions if s["quest_id"] == quest_id)


def get_quest_lap_history(sessions: list[dict], quest_id: str) -> dict:
...
def get_quest_segment_journey(sessions: list[dict], quest_id: str) -> list[dict]:
...
def get_today_timeline(sessions: list[dict]) -> list[dict]:
...
def get_berserker_stats(sessions: list[dict]) -> dict:
