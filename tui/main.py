...
from core.config import POMO_CONFIG, VALID_SOURCES
from core.utils import format_duration, fantasy_date, get_elapsed
...
from core.storage.json_backend import JsonQuestRepo, JsonPomoRepo

from tui.renderers import POMO_RPG_HEADLINERS
from tui.modals import (
    AddQuestModal, ConfirmModal,
    DashboardModal, DailyReceiptModal,
)
from tui.pomo_panel import PomodoroPanel
from tui.quest_panel import RosterPanel
...
class QuestLogApp(App):
    ...
    def _refresh_all(self) -> None:
        ...
        pomo_counts = get_all_pomo_counts_today(_pomo_repo.load_all())
    ...
    def _update_stats(self, quests: list[dict]) -> None:
        ...
        receipt_n  = len(get_today_receipt(_pomo_repo.load_all()))
    ...
    def _end_segment(self, completed: bool,
        ...
        self._pomo_session = _pomo_repo.get_session(self._pomo_session["id"])
    ...
    def _handle_deed_submit(self, deed: str, forge_type: str | None = None) -> None:
        ...
        _pomo_repo.update_segment_deed(self._pomo_session["id"], self._pomo_deed_lap, deed,
    ...
    def _stop_pomo(self, quiet: bool = False) -> None:
        ...
        self._clear_break_nudge()
        if self._pomo_session:
            _pomo_repo.end_session(self._pomo_session["id"])
            title  = self._pomo_session["quest_title"]
            actual = self._pomo_session.get("actual_pomos", 0)
    ...
    def action_start_quest(self) -> None:
        ...
        if quest["status"] not in VALID_SOURCES["start"]:
            ...
            return
        _quest_repo.update_status(quest["id"], "active")
        self._refresh_all()
    ...
    def action_block_quest(self) -> None:
        ...
        if quest["status"] not in VALID_SOURCES["block"]:
            ...
            return
        _quest_repo.update_status(quest["id"], "blocked")
        self._refresh_all()
    ...
    def action_done_quest(self) -> None:
        ...
        if quest["status"] not in VALID_SOURCES["done"]:
            ...
            return
        _quest_repo.update_status(quest["id"], "done")
        self._refresh_all()
    ...
    def action_toggle_frog(self) -> None:
        ...
        if quest is None:
            self.notify("No quest selected", severity="warning", timeout=2)
            return
        updated = _quest_repo.toggle_frog(quest["id"])
    ...
    def action_pomo(self) -> None:
        ...
        prior_pomos = get_quest_pomo_total(_pomo_repo.load_all(), quest["id"])
        self._pomo_lap_history = get_quest_lap_history(_pomo_repo.load_all(), quest["id"])
        self._pomo_lap         = prior_pomos
        self._pomo_is_resume   = prior_pomos > 0
        self._pomo_headliner   = random.choice(POMO_RPG_HEADLINERS)
        self._pomo_session     = _pomo_repo.start_session(quest["id"], quest["title"])
        self._refresh_all()
